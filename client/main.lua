local spawnLocationIndex = _('lastlocation')
...
if true then
	...
	AddEventHandler('esx_multicharacter:SetupCharacters', function()
		...
		local offset = GetOffsetFromEntityInWorldCoords(playerPed, Config.Offset.x, Config.Offset.y, Config.Offset.z)
		DoScreenFadeOut(0)
	...
	AddEventHandler('esx_multicharacter:SetupUI', function(data, slots)
		...
		if Character == nil then
			...
			spawnLocationIndex = _('lastlocation')
	...
	AddEventHandler('esx:playerLoaded', function(playerData, isNew, skin)
		...
		if spawnLocationIndex ~= _('lastlocation') and not isNew then
			spawn = Config.SpawnLocations[spawnLocationIndex]
		end
...
RegisterNUICallback('SelectChar', function (char, cb)
	SetupCharacter(char)
	local playerPed = PlayerPedId()
	SetPedAoBlobRendering(playerPed, true)
	ResetEntityAlpha(playerPed)
...
RegisterNUICallback('play-char', function (data, cb)
	spawnLocationIndex = data.SpawnLocation

	TriggerServerEvent('esx_multicharacter:CharacterChosen', data.char, false)

    cb('ok')
end)

RegisterNUICallback('delete-char', function (char, cb)
	TriggerServerEvent('esx_multicharacter:DeleteCharacter', char)
	spawned = false

    cb('ok')
end)

RegisterNUICallback('create-char', function(data, cb)
	ESX.UI.Menu.CloseAll()
...
TriggerEvent('chat:addSuggestion', '/forcelog', 'Force relog a player', {
    { name="id", help=" " }
})


TriggerEvent('chat:addSuggestion', '/ck', 'Character kill a player', {
    { name="id", help=" " }
})

TriggerEvent('chat:addSuggestion', '/unck', 'Character kill a player', {
    { name="number", help=" " }
})