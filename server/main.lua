...
elseif true == true then
	...
	if next(ESX.Players) then
		-- Clone the table manually
		local players = {}
		for k, v in pairs(ESX.Players) do
			players[k] = v
		end
	
		-- Wipe the original table
		for k in pairs(ESX.Players) do
			ESX.Players[k] = nil
		end
	
		-- Rebuild the ESX.Players table
		for _, v in pairs(players) do
			local identifier = GetIdentifier(v.source) -- Ensure this function exists
			if identifier then
	...
	local function SetupCharacters(source)
		...
		if result then
			...
			for i = 1, characterCount, 1 do
				...
				local playTime = math.floor(getHoursIdentifier(v.identifier) / 60) .. " hours"
				local last_seen = getLastSeen(v.identifier)

				characters[id] = {
					id = id,
					bank = accounts.bank and ESX.Math.GroupDigits(accounts.bank) or "0",
					money = accounts.money and ESX.Math.GroupDigits(accounts.money) or "0",					
					job = job,
					job_grade = grade,
